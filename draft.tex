...
%
Thus we seek to determine a set of initial conditions for which all possible trajectories of (\ref{eq:system}) subject to (\ref{eq:disturbance}) satisfy the robust constraints~(\ref{eq:state_input:constraints}) and probabilistic constraint (\ref{eq:probabilistic:constraint}). Denoting the largest such set as $\X^\infty$ we have
\begin{multline*}
\X^\infty = \Bigl\{x\in\X: \\
\begin{aligned}
...
% \end{equation}
%
Then from Proposition~\ref{prop1}, the inclusion $X^\infty\subseteq\X^\infty$ necessarily holds for any $\V^\prime\subseteq\V$ with probability measure $\PP\{\V^\prime\} \geq p$. Furthermore, by maximising the volume of $X^\infty$, we can improve the approximation of $\X^\infty$.
...
An optimisation program to approximate the positively invariant set $\X^\infty$ can therefore be formulated as
%
...
\end{subequations}
%
In section~\ref{sec:volume:of:hypercube} we discuss how this optimisation can be solved using parallelotopes for~$\V^\prime$.
...
%
That is the set of initial conditions $x_0\in\C_n$ such that a sequence of control inputs exists that drives the state in $n$ steps to the target set, $x_n\in\T$, for all admissible disturbance sequences 
...
\begin{aligned}
&x_k = A^k x_0 + \sum_{i=1}^{k} A^{k-i}B u_{k-i}(x_{k-i}) + \sum_{i=1}^{k} A^{k-i} w_{k-i}\\
&x_n\in\T\\
&x_k\in\X\\
&u_k(x_k)\in\U\\
&\PP\bigl\{v\in\V : Cx_k + Du_k(x_k) +v \in\Y\bigr\}\geq p
\end{aligned}\\
\forall w_k\in\W,\, \forall k\in\{ 0, \ldots , n\} \Bigr\}	
\end{multline*}
% \begin{equation}
...
% \end{equation}
%
Again, it is easy to verify that the set
%
\begin{multline*}
C_n(\T) = \Bigl\{ x\in\X: \exists \bigl\{u_0(\cdot),\ldots,u_{n-1}(\cdot)\bigr\}\\
\begin{aligned}
&x_k = A^k x_0 + \sum_{i=1}^{k} A^{k-i}B u_{k-i}(x_{k-i}) + \sum_{i=1}^{k} A^{k-i} w_{k-i}\\
&x_N\in\T\\
&x_k\in\X\\
...
\end{aligned}\\
\forall w_j\in\W,\, \forall v_k\in \V^\prime,\, \forall k\in\{0,\dots,n\} \Bigr\}	
\end{multline*}
% \begin{equation}
% 	C_n(\T) = \left\{x\in\X:\begin{aligned}&x_k = (A+BK)^k x + \sum_{i=1}^{k} (A+BK)^{k-i-1} w_{k-i}\\
% 	&x_N\in\T\\
...
% \end{equation}
%
is necessarily contained in $\C_n(\T)$ for any $\V^\prime\subseteq\V$ with measure $\PP\{\V^\prime\} \geq p$.
%
As in the case of the positively invariant set, fixing $\V^\prime$ to be a polytopic set leads to a computationally tractable optimisation for the maximisation of~$C_n$. This suggests an algorithm for constructing $C_n$ as a polytopic approximation of~$\C_n$ by optimising~$\V^\prime$ so as to maximise the volume of~$C_n$.
...
%
To achieve the best possible performance while guaranteeing constraint satisfaction a closed loop min-max model predictive control problem should be formulated, see e.g.~\cite{Lee:1997}.
%
...
%
%
In this section we present a method that enables optimisation over polytopes, as is required to solve~\eqref{seq:optimisation:MRPI:abstract} and~\eqref{seq:optimisation:n:step:set:abstract}.
%
...
%
First we illustrate the approximation of the guaranteed positively invariant set~$X^\infty$ for the system
%
...
\caption{The guaranteed positively invariant set of the system~\eqref{eq:example:system:MRPI} in red, blue the robust positively invariant set for the disturbance set computed with the methods discussed in~\cite{Zhang2015}.}
...
%
%
In this paper we presented a method in which the positively invariant set and the $n$-step controllable set of a stochastically constrained system are approximated respectively.
%
...
%
Although the presented scheme scales well to higher dimensions, the complexity of positively invariant sets and $n$-step controllable sets does not.
%
