...
Assume that $K$ is chosen so that $\rho < 1$, where $\rho$ is the spectral radius of $A+BK$. Also let $\Gamma$ be a matrix such that $\Gamma x\leq\bfa{1}$ and $-\Gamma x\leq\bfa{1}$ for all $x\in X_0$ and let $\bigl((A+BK),\Gamma\bigr)$ be observable, then the iteration~\eqref{eq:MRPI:sequence} %terminates after a finite number, $N$, of iterations, such that 
...
%, where $\rho$ denotes the spectral radius of $A+BK$, 
%such a matrix can be found using semi-definite programming.
%
...
%\textcolor{red}{Did we not say that 5 sentences ago?}
%
...
% % The additional sets are remaining $C_1(X^\infty),C_2(X^\infty)$ and $C_3(X^\infty)$ dashed for the comparison set obtained with~\eqref{eq:comparison:set} and solid for the optimised~$\V^\prime$ obtained with the proposed method.}
% % \caption{Approximation of the guaranteed 3-step controllable set for a box $\T$ (in blue). 
...
%
%The method we present is based on a simple restriction of the set $\V^\prime$ to be a parallelotope, which yields remarkable results in simulation and does not introduce non-convex constraints in the optimisation presented.
%
